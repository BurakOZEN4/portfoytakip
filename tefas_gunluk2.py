"""
TEFAS Günlük & Yılbaşı Fiyat Çekici + GitHub Otomatik Yükleme
───────────────────────────────────────────────────────────────
Her çalıştığında:
  1. Güncel fiyatları çeker
  2. Her yılın 01 Ocak fiyatlarını çeker
  3. fiyatlar.json'u günceller
  4. GitHub'a otomatik yükler → site telefondan erişilebilir olur
"""

import requests
import json
import time
import base64
from datetime import datetime, timedelta
from pathlib import Path

# ─── AYARLAR ─────────────────────────────────────────────────
FONLAR = ["KBJ", "HMK", "HKP"]
CIKTI_DOSYA = Path(__file__).parent / "fiyatlar.json"

# GitHub Ayarları — TOKEN'ı buraya yapıştır
GITHUB_TOKEN    = "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"
GITHUB_KULLANICI = "BurakOZEN4"
GITHUB_REPO     = "portfoytakip"
GITHUB_DOSYA    = "fiyatlar.json"   # Repodaki dosya adı
# ─────────────────────────────────────────────────────────────

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
    "Referer": "https://www.tefas.gov.tr/FonAnaliz.aspx",
    "X-Requested-With": "XMLHttpRequest",
    "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
}
MAX_DENEME = 4
BEKLEME_SN = 5

def fiyat_cek(fon_kodu, tarih):
    tarih_dt = datetime.strptime(tarih, "%Y-%m-%d")
    for delta in range(7):
        deneme_dt = tarih_dt - timedelta(days=delta)
        deneme_str = deneme_dt.strftime("%d.%m.%Y")
        url = "https://www.tefas.gov.tr/api/DB/BindHistoryInfo"
        payload = {"fontip":"YAT","bastarih":deneme_str,"bittarih":deneme_str,"fonkod":fon_kodu}
        for deneme in range(1, MAX_DENEME + 1):
            try:
                r = requests.post(url, data=payload, headers=HEADERS, timeout=20)
                r.raise_for_status()
                kayitlar = r.json().get("data", [])
                if kayitlar:
                    return float(kayitlar[0].get("FIYAT", 0)), deneme_str
            except Exception:
                if deneme < MAX_DENEME:
                    time.sleep(BEKLEME_SN)
    return None, None

def guncel_fiyat_cek(fon_kodu):
    bugun = datetime.now().strftime("%Y-%m-%d")
    return fiyat_cek(fon_kodu, bugun)

def yilbasi_fiyatlari_cek(fon_kodu, mevcut):
    buYil = datetime.now().year
    ilkYil = 2024
    yil_fiyatlar = {}
    mevcut_yil_fiy = mevcut.get("yillikFiyatlar", {}).get(fon_kodu, {})

    for yil in range(ilkYil, buYil + 1):
        tarih_key = f"{yil}-01-01"
        if tarih_key in mevcut_yil_fiy and yil < buYil:
            yil_fiyatlar[tarih_key] = mevcut_yil_fiy[tarih_key]
            print(f"   📂 {tarih_key}: {mevcut_yil_fiy[tarih_key]:.6f} TL (önbellekten)")
            continue
        print(f"   ⏳ {tarih_key}...", end=" ", flush=True)
        fiyat, gercek_tarih = fiyat_cek(fon_kodu, tarih_key)
        if fiyat:
            yil_fiyatlar[tarih_key] = fiyat
            print(f"✅ {fiyat:.6f} TL ({gercek_tarih})")
        else:
            if tarih_key in mevcut_yil_fiy:
                yil_fiyatlar[tarih_key] = mevcut_yil_fiy[tarih_key]
                print(f"⚠️  eski değer korundu")
            else:
                print(f"❌ alınamadı")
        time.sleep(1)
    return yil_fiyatlar

# ─── GITHUB YÜKLEYİCİ ────────────────────────────────────────
def github_yukle(json_icerik):
    if GITHUB_TOKEN == "BURAYA_TOKEN_YAPISTIR":
        print("\n⚠️  GitHub token girilmemiş, yükleme atlandı.")
        return False

    url = f"https://api.github.com/repos/{GITHUB_KULLANICI}/{GITHUB_REPO}/contents/{GITHUB_DOSYA}"
    headers = {
        "Authorization": f"token {GITHUB_TOKEN}",
        "Accept": "application/vnd.github.v3+json",
    }

    # Mevcut dosyanın SHA'sını al (güncelleme için gerekli)
    sha = None
    try:
        r = requests.get(url, headers=headers, timeout=15)
        if r.status_code == 200:
            sha = r.json().get("sha")
    except Exception:
        pass

    # Dosyayı base64'e çevir
    icerik_b64 = base64.b64encode(json_icerik.encode("utf-8")).decode("utf-8")
    tarih_str = datetime.now().strftime("%d.%m.%Y %H:%M")
    payload = {
        "message": f"Otomatik güncelleme: {tarih_str}",
        "content": icerik_b64,
    }
    if sha:
        payload["sha"] = sha

    try:
        r = requests.put(url, headers=headers, json=payload, timeout=20)
        if r.status_code in (200, 201):
            print(f"✅ GitHub'a yüklendi!")
            print(f"   🌐 https://{GITHUB_KULLANICI}.github.io/{GITHUB_REPO}/portfolyo.html")
            return True
        else:
            print(f"❌ GitHub hatası: {r.status_code} — {r.json().get('message','')}")
            return False
    except Exception as e:
        print(f"❌ GitHub bağlantı hatası: {e}")
        return False

# ─── ANA FONKSİYON ───────────────────────────────────────────
def main():
    print("=" * 50)
    print(f"  TEFAS Fiyat Çekici")
    print(f"  {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
    print("=" * 50)

    if CIKTI_DOSYA.exists():
        with open(CIKTI_DOSYA, "r", encoding="utf-8") as f:
            mevcut = json.load(f)
    else:
        mevcut = {"guncellendi": "", "fiyatlar": {}, "yillikFiyatlar": {}}

    if "yillikFiyatlar" not in mevcut:
        mevcut["yillikFiyatlar"] = {}

    yeni_fiyatlar = {}
    yeni_yillik = {}

    for fon in FONLAR:
        print(f"\n{'─'*40}")
        print(f"  {fon}")
        print(f"{'─'*40}")

        print(f"📡 Güncel fiyat...", end=" ", flush=True)
        fiyat, tarih = guncel_fiyat_cek(fon)
        if fiyat:
            print(f"✅ {fiyat:.6f} TL ({tarih})")
            yeni_fiyatlar[fon] = {"fiyat": fiyat, "tarih": tarih, "ccy": "TRY"}
        else:
            print(f"❌ Alınamadı")
            if fon in mevcut.get("fiyatlar", {}):
                yeni_fiyatlar[fon] = mevcut["fiyatlar"][fon]
                print(f"   → Eski fiyat korundu")

        print(f"\n📅 Yılbaşı fiyatları:")
        yeni_yillik[fon] = yilbasi_fiyatlari_cek(fon, mevcut)

    # JSON güncelle
    mevcut["guncellendi"] = datetime.now().isoformat()
    mevcut["fiyatlar"] = yeni_fiyatlar
    mevcut["yillikFiyatlar"] = yeni_yillik

    json_str = json.dumps(mevcut, ensure_ascii=False, indent=2)

    # Yerel kaydet
    with open(CIKTI_DOSYA, "w", encoding="utf-8") as f:
        f.write(json_str)

    print(f"\n{'='*50}")
    print(f"✅ fiyatlar.json güncellendi!")

    # GitHub'a yükle
    print(f"\n🚀 GitHub'a yükleniyor...")
    github_yukle(json_str)

    print(f"{'='*50}\n")

if __name__ == "__main__":
    main()
